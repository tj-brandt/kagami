...
// --- Constants ---
const QUALTRICS_SURVEY_BASE_URL = process.env.REACT_APP_QUALTRICS_SURVEY_BASE_URL;
...
function App() {
  ...
  useEffect(() => {
    ...
    if (isDemoMode) {
      ...
      setParticipantDetails(demoParticipantId, null);
      ...
      if (pidFromUrl && avatarTypeFromUrl && lsmTypeFromUrl && responseIdFromUrl) {
        const AVATAR_WHITELIST = new Set(['none', 'premade', 'generated']);
        const LSM_WHITELIST = new Set(['static', 'adaptive']);

        if (!AVATAR_WHITELIST.has(avatarTypeFromUrl) || !LSM_WHITELIST.has(lsmTypeFromUrl)) {
            console.error("CRITICAL: Invalid avatar or lsm URL parameters.");
            logEvent('invalid_url_params', { params: window.location.search, reason: 'Whitelist failed' });
            setPhase('error');
            return;
        }

            if (!QUALTRICS_SURVEY_BASE_URL) {
              console.error("CRITICAL: REACT_APP_QUALTRICS_SURVEY_BASE_URL is not defined in the environment.");
              logEvent('configuration_error', { error: 'Qualtrics URL missing' });
              setPhase('error');
              return;
          }

          const returnUrl = `${QUALTRICS_SURVEY_BASE_URL}?Q_R=${encodeURIComponent(responseIdFromUrl)}&ReturnedFromApp=true`;
        ...
        api.startSession(pidFromUrl, conditionName)
          ...
          logEvent('invalid_url_params', { params: window.location.search, reason: 'Missing params' });
          setPhase('error');
      }
    }
